      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column label="打款信息" >
            <template slot-scope="scope">
              <span v-if="scope.row.pay_type.value==10">姓名:{{scope.row.account.alipay_name}}</br> 账号:{{scope.row.account.alipay_account}}</span>
              <span v-if="scope.row.pay_type.value==20">姓名:{{scope.row.account.bank_account}}</br>开户行:{{scope.row.account.bank_name}} </br>账号:{{scope.row.account.bank_card}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="apply_status" label="状态">
            <template slot-scope="scope">
              ...
              <div v-if="scope.row.apply_status.value==30">驳回 <span class="red">原因：{{scope.row.reject_reason}}</span></div>
