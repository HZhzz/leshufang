  ...
  -->
    <div class="user">
            ...
            <el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
                ...
                <el-form-item label="门店地址">
                    <el-select size="small" v-model="searchForm.store_id" placeholder="请选择">
                        <el-option label="全部" value=""></el-option>
                        <el-option v-for="(item, index) in shopList" :key="index" :label="item.store_name" :value="item.store_id"> </el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item>
                    <el-button size="small" type="success" @click="onExport" v-auth="'/order/order/operate/export'">导出 </el-button>
                </el-form-item>
                <el-form-item style="float: right;">
                    <el-button size="small" type="warning" @click="showBatchDelivery" v-auth="'/order/order/operate/batchDelivery'"
                        >批量发货</el-button
                    >
                </el-form-item>
            </el-form>
            ...
            <div class="table-wrap">
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    ...
                    <el-tab-pane :label="'未付款'" name="payment">
                        <span slot="label"
                            >未付款 <el-tag size="mini">{{ order_count.payment }}</el-tag></span
                        >
                    </el-tab-pane>
                    <el-tab-pane :label="'待发货'" name="delivery">
                        <span slot="label"
                            >待发货 <el-tag size="mini">{{ order_count.delivery }}</el-tag></span
                        >
                    </el-tab-pane>
                    <el-tab-pane :label="'待收货'" name="received">
                        <span slot="label"
                            >待收货 <el-tag size="mini">{{ order_count.received }}</el-tag></span
                        >
                    </el-tab-pane>
                    <el-tab-pane :label="'待取消'" name="cancel">
                        <span slot="label"
                            >待取消 <el-tag size="mini">{{ order_count.cancel }}</el-tag></span
                        >
                    </el-tab-pane>
                ...
                <el-table size="small" :data="tableData.data" :span-method="arraySpanMethod" border style="width: 100%" v-loading="loading">
                        ...
                        <template slot-scope="scope">
                            ...
                            <template v-else>
                                <div class="product-info" v-for="(item, index) in scope.row.product" :key="index">
                                    ...
                                    <div class="d-c-c d-c">
                                        <div class="orange">￥ {{ item.total_pay_price }}</div>
                    ...
                    <el-table-column prop="" label="买家">
                        <template slot-scope="scope" v-if="!scope.row.is_top_row">
                            ...
                            <div class="gray9">ID：({{ scope.row.user.game_user_id }})</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column fixed="right" label="操作" width="210">
                        <template slot-scope="scope" v-if="!scope.row.is_top_row">
                            ...
                            <el-button
                                v-if="scope.row.delivery_type.value == 10 && scope.row.delivery_status.value == 20"
                            ...
                            <el-button
                                v-if="
                                    scope.row.pay_status.value == 20 &&
                                        ...
                                        scope.row.order_status.value != 20 &&
                                        scope.row.order_status.value != 21 &&
                            ...
                            <el-button
                                v-if="
                                    scope.row.pay_status.value == 20 &&
                                        ...
                                        scope.row.order_status.value != 20 &&
                                        scope.row.order_status.value != 21 &&
                                        scope.row.delivery_type.value == 30
                                "
                                @click="addClick(scope.row)"
                                type="text"
                                size="small"
                            ...
                            <el-button
                                v-if="scope.row.pay_status.value == 20 && scope.row.delivery_status.value == 10 && scope.row.order_status.value == 10"
                                @click="cancelClick(scope.row)"
                                type="text"
                                size="small"
                                v-auth="'/order/order/operate/confirmCancel'"
                            >
                                取消订单
                            </el-button>
                            <el-button
                                v-if="scope.row.order_status.value == 21"
                                @click="addClick(scope.row)"
                                type="text"
                                size="small"
                                v-auth="'/order/order/operate/confirmCancel'"
        ...
        <el-dialog title="批量发货说明" :visible.sync="dialogBatchDelivery" width="30%">
            ...
            <div>3、请检查物流公司是否已经添加到系统，<el-link type="primary" :underline="false" @click="gotoExpress()">查看</el-link></div>
