	...
	<view>
		...
		<view v-if="total>0" style="padding: 20rpx;">
			<view class="boxitem" v-for="(item,index) in goodList" :key="index">
				<view>
					<view style="font-size: 28rpx;color: #333;font-weight: bold;">订单号:<text style="font-size: 26rpx;color: #666;font-weight: 100;">（{{item.order_no}})</text></view>
					<view style="padding-top: 32rpx;">{{item.create_time}}</view>
					<view style="padding-top: 32rpx;" v-if="item.jion_id">
						订单号:{{item.jion_id}}
					</view>
				</view>
				<view>
					<text style="font-size: 36rpx;color: #EF4F4F;font-weight: bold;" >+{{item.income_price}}</text>
					<!-- <text style="font-size: 36rpx;color:#65B56D ;font-weight: bold;" v-if="item.type=='2'">-{{item.total_price}}</text> -->
				</view>
			</view>
		</view>
		<view v-else>
...
<script>
	export default{
		...
		methods:{
			...
			homeinfoList(){
				this.$api.getUserTotalPriceLog(this.queryData).then(res=>{
